...
public class Application2 {
    ...
    public static void main(String[] args) {
        ...
        *   이것을 참조하여 사용하기 때문에 참조자료형(reference type) 이라고 한다. */
        ...
        // iarr = new int[]; 공간의 갯수를 지정하지 않으면 에러 발생
        iarr = new int[5];    // new 를 선언해주는 순간 heap에 []안의 갯수만큼 공간을 만들어줌
        carr = new char[10];

        // int x; x = 10;

        /* comment. 선언과 동시에 할당을 할 수도 있다. */
        int[] iarr2 = new int[5];
        ...
        *   stack 에 저장된 주소로 heap 에 할당이 된 배열을 찾아갈 수 있다. */
        ...
        /* comment.
        *   hashcode() : 일반적으로 객체의 주소값을 10진수 => 우리가 알 수 있는 정수로 바꾸어주는 메소드
        *                   -> 동일객체인 지 비교할 때 주로 쓰인다. */
        ...
        *   length
        *   - 필드는 뒤에서 다룰거지만, 일종의 변수라고 새각하면 된다.
        ...
        *   다만, 레퍼런스변수를 null 로 변경하여 더 이상
        *   주소를 참조할 수 없게 된 배열은 일정 시간이 지나면
        ...
        *   참조하고 있는 경우에 참조연산자(.) 을 사용하게 될 때 발생하는 에러(가장 많이 볼 에러중 1개이다) */
