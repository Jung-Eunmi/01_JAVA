...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        *   클래스들의 집합을 의미한다.
        *   즉, 누군가가 만들어 둔 데이터를 효율적으로 저장하는 자료구조와
        ...
        *    - key 는 위에서 나온 Set 방식으로 관리하기 때문에 데이터의 순서를
        *      관리하지 않고, key 값은 중복을 허용하지 않는다.★
        ...
        *     -> 내부적으로 배열을 이용해서 요소를 관리하며,
        *        인덱스를 이용해 배열의 요소에 빠르게 접근을 할 수 있다.
        * */
        ...
        *    - 크기 변경, 요소의 추가, 삭제, 정렬 기능을
        *      미리 메소드로 만들어서 우리에게 제공을 해주고 있다.
        * */

        // ArrayList 는 인스턴스를 생성하면 10칸 짜리의 배열을 만들어준다.
        ArrayList alist = new ArrayList();  // ArrayList 의 기능만 사용할 수 있다.
...
//        List list = new List();
        ...
        *   레퍼런스 타입은 List 로 하는 것이 더 유연한 코드를 작성하는 방법이다. */
        List list = new ArrayList(); // List인 부모의 기능과 ArrayList 인 자식의 기능까지 사용할 수 있다.

        // list 에 값을 추가하는 방법
        alist.add("apple");
        alist.add(123);         // auto-boxing 처리가 됨
        alist.add(45.67);       // int, double 이러한 기본(원시)값을 객체 - Integer, Double 로 바꿔줌
        alist.add(new Date());

        // toSting 메소드가 Overriding 되어 있기 때문에
        // 주소값이 출력이 안 되고, 값이 나오고 있다.
        // 출력해보면 우리가 작성한 순서대로 값이 들어있는 것을 확인 할 수 있다.
        System.out.println("alist = " + alist);
        System.out.println("alist 의 크기 : " + alist.size());

        // 인덱스로 관리를 한다. -> for 문(반복문)을 사용해서 규칙적으로 접근 가능하다
...
//        stringList.add(123);
        ...
        *   List 의 또 다른 구현체인 LinkedList 에서 제공을 한다. */
        // List<Sting> stringList = new LinkedList<>(stringList);
        ...
        *   없기 때문에 인덱스를 사용하지 않고도 반복문을 쓰기 위한 목록을 만들어주는 역할이라고 보면된다.
