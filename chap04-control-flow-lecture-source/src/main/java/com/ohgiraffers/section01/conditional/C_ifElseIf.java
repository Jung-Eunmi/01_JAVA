...
public class C_ifElseIf {
    ...
    /* title. if, if-else, if-else-if 문 단독 사용에 대한 흐름을 이해하고 적용할 수 있다. */

    /* comment.
    *   [if-else-if 문 표현식]
    *   if(조건식1) {
    *       조건식1 이 true 일 때 실행할 구문;
    *   } else if(조건식2) {
    *       조건식1 이 false 이고 조건식2 가 true 일 때 실행할 구문;
    *   } else {
    *       위의 조건 2개가 모두 false 일 경우 실행할 구문;
    *   }
    * */

    /* comment.
    *   if-else-if 문은 조건식1 의 결과값이 참(true) 이면 if { } 블럭 실행,
    *   조건식1 이 false 이면 조건식2 를 확인하여 참(true) 이면 else if { } 블럭을 실행한다.
    *   조건식1,2 의 결과가 모두 거짓(false) 이면 else { } 블럭을 실행한다.
    * */
    ...
    public void testNestedIfElseIfStatement() {
        ...
        *   학생의 이름과 점수를 입력 받아
        *   90점 이상이면 'A', 80점 이상이면 'B', 70점 이상이면 'C', 60점 이상이면 'D', 60점 미만이면 'F' 학점을 줄 것이다.
        *   추가로 각 등급의 중간점수 (ex. 65,75,85,95) 이상이면 '+' 를 붙여 줄 것이다.
        * */
    ...
    public void improveNestedIfElseIfStatement() {
        ...
        // 각 등급의 중간 점수 이상이면서 60점 이상인 경우 + 를 붙이는 복잡하지 않은 방법
