...
public class OverloadingTests {
    ...
    *   동일한 이름으로 다양한 종류의 매개변수에 따라서 처리해야 하는 여러 메소드를
    *   관리하는 기술을 오버로딩(overloading) 이라고 한다.
    * */

    /* comment.
    *   오버로딩의 조건
    *   동일한 이름을 가진 메소드의 파라미터 선언부 매개변수의 타입, 갯수, 순서를
    *   다르게 작성해서 한 클래스 내에 동일한 이름의 메소드를 여러 개 작성할 수 있도록 한다.
...
//    public void test() {}
    ...
    // 접근제한자를 바꾸어도 메소드 시그니처가 같기 때문에 컴파일에러 발생
//    private void test() {}

    // 반환형으로 바꾸어도 메소드 시그니처가 같기 때문에 컴파일에러 발생
//    public int test() {return 0;}

    // 메소드 이름은 동일하지만 시그니처가 같지 않기 때문에 컴파일에러가 발생하지 않음
