...
public class Application3 {
    ...
    public static void main(String[] args) {
        ...
        /* title. Scanner 사용 시 주의사항에 대해 이해 할 수 있다. */

        /* comment.
        *   스캐너의 next 메소드들은 입력한 내용을 버퍼로부터 토큰 단위로 분리해서 읽어오게 된다.
        ...
        *   hi만 입력하고 정수를 입력하면 정상적으로 동작한다.
        *   하지만, hi world 를 입력하게 되면 정수를 입력하기도 전에 InputMismatch 오류가 발생한다.
        ...
        * */
        ...
        /* comment.
        *   입력을 받을 시 nextLine() 을 호출한 것이 아직 행을 바꾸지 않고 한 라인을
        *   읽어드린다고 판단을 해서 아무것도 읽지 못하고 넘어간 뒤에 다음 출력 구문을 실행 한 것이다.
        *   이걸 해결하기 위해서는 sc.nextLine() 호출 전에 미리 sc.nextLine() 을 한 번 호출하면
        *   그 행을 읽어드리는 걸 다음 행으로 넘기게 되고, 그 이후 새로 입력 받은 한 라인을 읽어와서 str2에 저장하게 된다.
        * */
        sc.nextLine();   // InputMismatch 오류가 발생할 때 호출 전에 쓰기! 이것만 알아놓고 현업에서 쓰지 않으니 공부 안해도됨
