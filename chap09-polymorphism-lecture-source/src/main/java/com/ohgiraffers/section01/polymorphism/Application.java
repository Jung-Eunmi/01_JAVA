...
//        개구리 f1 = new 동물();
        ...
        *   런타임 시 실제 객체를 가진 오버라이딩 된 메소드로 바인딩이 바뀌어 동작하는 것을 의미한다.
        * */
        ...
        /* comment.
        *   하지만 현재 레퍼런스 변수의 타입은 동물이기 때문에
        *   너구리와 개구리가 가진 고유의 기능은 작동시킬 수 없다.
        * */
//        a1.낼름공격();

        /* index 3. 타입 형변환 확인 */
        System.out.println("==============클래스타입 확인==============");
        ((개구리)a1).낼름공격();
        ((너구리)a2).깨물기();

        // 주의사항 : 타입 형변환을 잘못하게 되면 컴파일 시에는 문제없지만, 런타임 시 ClassCast 오류 발생
//        ((너구리)a1).깨물기();
        ...
        System.out.println("a1 이 동물인 지 확인 : " + (a1 instanceof 동물));
        ...
        * */
        ...
        너구리 a4 = (너구리)new 동물(); // down-casting (강제형변환과 비슷함)
